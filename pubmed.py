...
def pubmed_query(search=True, **kwargs):
	...
	params = dict({'retmode': 'xml', 'rettype': 'Abstract',
				   'db': 'pubmed', 'retmax': config['pubmed_query_limit']},
				   **kwargs)

	# create url serializing kwargs in GET
	url = 'http://eutils.ncbi.nlm.nih.gov/entrez/eutils/e%s.fcgi?%s' % ('search' if search else 'fetch',
